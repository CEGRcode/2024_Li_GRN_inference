...
def Multi_GenerateMutation(GRN_instances_dict, instance_key, indexes_of_diff_gene):
    ...
    if GRN_instances.f0_c[indexes_of_diff_gene[j]][:2] in [[0, 0], [1, 1]]:
        ...
        sys_cache_00[j] = {key[3:]: value for key, value in sys_cache[j].items() if key.startswith('00\t')}
        sys_cache_11 = copy.deepcopy(sys_cache)
        sys_cache_11[j] = {key[3:]: value for key, value in sys_cache[j].items() if key.startswith('11\t')}
