...
def Find_intermediate_TFs(chr_, left_, right_, YEP_ID):
    output_ = []
    bed_file = open('./data/{}_YEP/{}_chexmix_filtered_peaks.bed'.format(YEP_ID, YEP_ID),'r')
...
for each in ssTF_names:
    if not os.path.isfile('./data/{}_YEP/{}_chexmix_filtered_peaks.bed'.format(YEP_ID_dic[each.title().upper()], YEP_ID_dic[each.title().upper()])):
