#!/bin/bash
# ---------------------- MAKE COMPOSITE PROFILE FOR EACH PROTEIN ITSELF MOTIF-BASED PPI -----------------
...
for folder in ./*_YEP/; do
	...
	# Get the motif bed file covering most peaks
	bed_with_most_lines=$(wc -l ${folder}${ID}_Motif_*_bound.bed | sort -nr | head -n 2 | tail -n 1 | awk '{print $2}')
	motif_number=$(basename "$bed_with_most_lines" | grep -oP 'Motif_\K\d+')
	...
	# expand the bed file.
	java -jar ./ScriptManager-v0.14.jar coordinate-manipulation expand-bed -c 500 "${bed_with_most_lines}" -o "${folder}${ID}_Motif_${motif_number}_peaks_500bp.bed"

	# remove the last two rows in the chexmix bed file.
	awk '{NF=NF-2}1' "${folder}${ID}_Motif_${motif_number}_peaks_500bp.bed" > "${folder}${ID}_Motif_${motif_number}_peaks_500bp_remove.bed"
	sed 's/$/\r/' "${folder}${ID}_Motif_${motif_number}_peaks_500bp_remove.bed" > "${folder}${ID}_Motif_${motif_number}_peaks_500bp_remove_temp.bed"
	sed 's/ /\t/g' "${folder}${ID}_Motif_${motif_number}_peaks_500bp_remove_temp.bed" > "${folder}${ID}_Motif_${motif_number}_peaks_500bp_remove_convert.bed"

	# pile up tags and output in cdt format.
	java -jar ./ScriptManager-v0.14.jar read-analysis tag-pileup "${folder}${ID}_Motif_${motif_number}_peaks_500bp_remove_convert.bed" "${folder}${ID}_filtered_sorted.bam" --cdt -M "${ID}_BED_${ID}_BAM"
	...
	rm "${folder}${ID}_Motif_${motif_number}_peaks_500bp_remove_temp.bed" "${folder}${ID}_Motif_${motif_number}_peaks_500bp_remove.bed" "${ID}_BED_${ID}_BAM_sense.cdt" "${ID}_BED_${ID}_BAM_anti.cdt" "${folder}${ID}_filtered.bam"
done