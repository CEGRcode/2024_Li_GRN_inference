...
def updateLGonAM(AM, LG):
    '''If a TF is not regulating a TG in AM, break any existing complexes having the TF on the TG in LG.'''
    TotalNumberOfGenes = int(np.sqrt(len(AM)))
    if type(LG) == str:
        LG_flatten = list(map(int, LG.split(',')))
        LG = [LG_flatten[i:i + TotalNumberOfGenes] for i in range(0, len(LG_flatten), TotalNumberOfGenes)]
    else:
        pass
    for i in range(0, len(AM)):
        TG_index = i % TotalNumberOfGenes
        TR_index = i // TotalNumberOfGenes
        if AM[i] == '0':
            if LG.count(LG[TG_index][TR_index]) == 1:
                pass
            else:
                LG[TG_index][TR_index] = 1 + max(LG[TG_index])
        else:
            pass    
    LG = Sort_LG(LG)
    return Matrix2String01_LG_Expanded(LG)

def token_generator(path, sep=',', chunk_size=65536):
    """
    Yield tokens split by `sep` from a large file, reading in chunks.
    Tokens are stripped of whitespace. Empty tokens are skipped.
    """
    with open(path, 'r', encoding='utf-8') as f:
        ...
        while True:
            ...
            buf += chunk
            parts = buf.split(sep)
            # yield all except the last (it may be partial)
            for tok in parts[:-1]:
                tok = tok.strip()
                if tok:
                    yield tok
...
def get_LG_for_gene(path, start_index, count, sep=',', as_int=True):
    """
    Return `count` tokens starting at `start_index` (0-based).
    as_int: convert tokens to int if True, otherwise return strings.
    """
    gen = token_generator(path, sep=sep)
    sliced = itertools.islice(gen, start_index, start_index + count)
    if as_int:
        return [int(x) for x in sliced]
    else:
        return list(sliced)
