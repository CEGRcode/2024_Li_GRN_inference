...
Full usage with optional prior files:
...
#### Optional arguments
- `-p`: p-value cutoff for discrete state calling (default: `0.01`)
...
This step writes the processed files mainly to `./data` and diagnostic plots to `./result`.

---

### Directly using your own prior matrices (skip preprocessing)

If you already have your own prior matrices, you can skip the JSON preprocessing step and directly use them in SETIA.

#### TF–DNA prior matrix
The TF–DNA prior should be a **square adjacency matrix** in `.txt` format.
...
- `1` means regulation is allowed
- `0` means no prior edge
- the matrix should be **N × N**, where **N = number of modeled genes**
- **row and column order must exactly match the gene order in the RNA-seq matrix**
- this same order should also match `GRN_ssTFs_column_names.txt`

#### LG prior (synergistic TF groups)
The LG prior is a plain text file where **each line corresponds to one target gene**.

Each line is a comma-separated string of TF group labels.

TFs with the **same number work together synergistically**.
...
This means all TFs work independently.
...
This means:
- TF 2 and TF 3 work together
- TF 5 and TF 6 work together
- all others work independently

So the numbers define **which TF indexes belong to the same regulatory complex for that gene**.

The number of entries in each line must match the total number of TFs.

A real example file is:
`./data/GRN_ssTFs_Sc_LG.txt`

### Core outputs generated by preprocessing
