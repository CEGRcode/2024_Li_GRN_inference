# ---------------------- MAKE COMPOSITE PROFILE FOR EACH PROTEIN TO ALL OTHER PROTEINS MOTIF-BASED PPI-------
...
for folder in ./*_YEP/; do
	...
	# get the primary motif bed of factor BED_ID
	Motif_bed=$(find ./${BED_ID}_YEP/ -maxdepth 1 -type f -name '*500bp_remove_convert.bed' -exec basename {} \;)
	...
	for folder in ./*_YEP/ ./*_YEP_Done/; do
		...
		java -jar ./ScriptManager-v0.14.jar read-analysis tag-pileup "${BED_ID}_YEP/${Motif_bed}" "${folder}${ID}_filtered_sorted.bam" --cdt -M "${BED_ID}_BED_${ID}_BAM"
	...
	echo "$BED_ID: $count  $Motif_bed" >> QC_count.txt # make sure each factor has been comparied to all other factors
