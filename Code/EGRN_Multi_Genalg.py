...
if __name__ == '__main__':
            ...
            Sigmoid_k_init.append(np.log(10**-3/(1-10**-3))
                                  / np.log(TranscriptionThreshold[i][0]
                                           / (TranscriptionPofileMax[i]*TranslationRate[i]
                                              / DegradationRateProtein[i])))
        if null_net:
            LogicGates = np.array([[0, 0] for i in range(0, TotalNumberOfGenes)])
            #LogicGates = LogicGatesString2Matrix('0000001000001111')
        else:
            LogicGates = np.array([np.random.randint(2, size=2).tolist() for i in range(0, TotalNumberOfGenes)])
    ...
    while Loopcounter <= int(sys_iteration_num):
        ...
        for SPP_i in range(0, len(WTTP)):
            ...
            for j in range(0, len(NewmRNA)):
                NewProtein[j] = eval('({}.TranslationRate[j]*NewmRNA[j])/({}.DegradationRateProtein[j]+{}.DilutionRate[j])'.format(GRN_List[0], GRN_List[0], GRN_List[0]))
            for j in range(0, SampleSize):
                exec('{}.SetmRNA({},{},{})'.format(GRN_List[j], 'NewmRNA', WTTP[str(SPP_i)][0], 'Overexpression[SPP_i]'))
                exec('{}.SetProtein({},{},{})'.format(GRN_List[j], 'NewProtein', WTTP[str(SPP_i)][0], 'Overexpression[SPP_i]'))
            for j in range(0, SampleSize):
            ...
            for j in range(0, len(expanded_index_to_train)):
                ...
                if IsPointAttractor:
                    ...
                    if expanded_index_to_train[j][1] == 0:
                        ...
                        CurrentDistance_2.append(GetAttractorDistance(Memo_mRNA,WTTP[str(i)][1],TranscriptionPofileMax))
                    ...
                    if expanded_index_to_train[j][1] == 0:
                        ...
                        CurrentDistance_2.append(0) # self-activation leads to non-fixed-point attractor, which must not be included in the input.
            IDic = {}
            for j in range(0, len(expanded_index_to_train)):
