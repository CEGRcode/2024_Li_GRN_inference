...
for Global_i in range(0, len(WTTP)):
    ...
    for Number_of_initial_positions in range(0, 1):
        ...
        scipystring = eval('{}.Delta_mRNA(WTTP[str(Global_i)][0], Overexpression[Global_i])'.format(
            GRN_List[0]))  # Calculate delta_mRNA
        # print(scipystring)
        exec(scipystring)
        sol = solve_ivp(update_mRNA_protein, [0, TrainingCount], list(NewmRNA) + list(NewProtein),
                        args=([WTTP[str(Global_i)][0]]), t_eval=[tick for tick in range(0, TrainingCount)])
        ########################################################################################################################################################################################
        ...
        for o in range(0, TotalNumberOfGenes):
            if np.std(npmRNA[npmRNA.shape[0] - 100:npmRNA.shape[0] + 1, o]) < 1:
                ...
                arithmetic_sequence = np.argwhere(npmRNA[TrainingCount - 100:TrainingCount + 1, o] == np.amax(
                    npmRNA[TrainingCount - 100:TrainingCount + 1, o]))
                diff_seq = []
    ...
    #plt.title('Gene expression simulation, N={}'.format(Number_of_initial_positions), fontsize=25)
    #plt.show()'''
...
print(WTTP)
print(npmRNA_continuous.shape, npmRNA_continuous[-1])