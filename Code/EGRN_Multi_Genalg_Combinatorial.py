...
if __name__ == '__main__':
    ...
    def ParametersInitiations(MutationRate_, which_i, null_net):
        ...
        if null_net:
            ...
            # RandomString = '1000000001000000001000000001000100001000000000000000001000000000'
        else:
            for INT in RandomStringList:
        ...
        # and the activators and repressors do not present simultaneously.
            ...
            Sigmoid_k_init.append(np.log(10 ** -3 / (1 - 10 ** -3))
                                  / np.log(TranscriptionThreshold[i][0]
                                           / (TranscriptionPofileMax[i] * TranslationRate[i]
                                              / DegradationRateProtein[i])))
        if null_net:
            LogicGates = np.array([[j for j in range(0, TotalNumberOfGenes)] for i in range(0, TotalNumberOfGenes)])
            # LogicGates = LogicGatesString2Matrix('0000001000001111')
        else:
            arlist = [[] for i in range(0, TotalNumberOfGenes)]
            for i in range(0, TotalNumberOfGenes):
                temp_list = []
                for j in range(0, TotalNumberOfGenes):
                    temp_list.append(np.where(Configuration[i][:, j] == 1)[0].tolist())
                arlist[i] = temp_list
            LogicGates = Mutation_LG(np.random.randint(TotalNumberOfGenes**2), np.array([[0 for j in range(0, TotalNumberOfGenes)] for i in range(0, TotalNumberOfGenes)]), arlist)
        ...
        for i in range(0, TotalNumberOfGenes):
            f0.append(
                (DegradationRatemRNA[i] * TranscriptionPofileAve[i] - Leakage[i]) / (TranscriptionRate[i] - Leakage[i]))
    ...
    while Loopcounter <= int(sys_iteration_num):
        ...
        for SPP_i in range(0, len(WTTP)):
            ...
            for j in range(0, SampleSize):
                exec(
                    '{}.SetmRNA({},{},{})'.format(GRN_List[j], 'NewmRNA', WTTP[str(SPP_i)][0], 'Overexpression[SPP_i]'))
                exec('{}.SetProtein({},{},{})'.format(GRN_List[j], 'NewProtein', WTTP[str(SPP_i)][0],
                                                      'Overexpression[SPP_i]'))
            for j in range(0, SampleSize):
                f0_matrix[SPP_i].append(eval('{}.Updatef0_1()'.format(GRN_List[j])))
                ...
                if IsPointAttractor:
                    ...
                    if expanded_index_to_train[j][1] == 0:
                        ...
                        CurrentDistance_2.append(
                            GetAttractorDistance(Memo_mRNA, WTTP[str(i)][1], TranscriptionPofileMax))
                    ...
                    Memo_mRNA = np.array([0.1 for g in range(0, TotalNumberOfGenes)])
                    if expanded_index_to_train[j][1] == 0:
                        Attractor_collector[str(expanded_index_to_train[j][0])].append(Memo_mRNA)
                        CurrentDistance_2.append(5 * TotalNumberOfGenes)  # This penalty isn't about self-activation.
                    else:
                        CurrentDistance_2.append(
                            0)  # self-activation leads to non-fixed-point attractor, which must not be included in the input.
            IDic = {}
            for j in range(0, len(expanded_index_to_train)):
                if expanded_index_to_train[j][0] not in IDic:
                    IDic[expanded_index_to_train[j][0]] = [CurrentDistance_2[j]]
                else:
                    IDic[expanded_index_to_train[j][0]].append(CurrentDistance_2[j])
        ...
        # Make a Long Jump
        if TimeToMakeALongJump >= 50:
            if GlobalMutationRate < (TotalNumberOfGenes * 2):
            ...
            for j in range(0, len(LongJumpIndex_List)):
                (Protein, Configuration, MutationRate,
                 ...
                 DilutionRate,
                 TranscriptionThreshold,
                 LogicGates, Sigmoid_ks, Leakages, f0) = ParametersInitiations(GlobalMutationRate, i, False)
                exec('{} = GRN(\'{}\', {},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{})'.format(
                    GRN_List[i], GRN_List[i],
                    'InitialTranscriptionProfile[i]',
                    'Protein',
        ...
        if round(meanDistance[-1], 5) < ((1 + 2 * SelectionPower) * round(minDistance[-1], 5)):
            ...
            # print('*** Recombination ***')
        # No good mutations occur, don't care if the population converges
        if minDistance[-1] >= minDistance_Single:
        ...
        OutputFile3 = open(
            sys_path + '/result/' + sys_output_name + '_ResultFlow.txt', 'a')
        CurrentProportions = []
        for j in range(0, SampleSize):
            CurrentProportions.append(
                eval('{}.Proportion'.format(GRN_List[j])))
    ...
    OutputFile = open(
        sys_path + '/result/' + 'BetaTest_G{}_Result.txt'.format(TotalNumberOfGenes), 'a')
    ...
    OutputFile = open(
        sys_path + '/result/' + 'BetaTest_G{}_Result_f0.txt'.format(TotalNumberOfGenes), 'a')
    TheBestGuy_Configuration = TheBestGuy.Configuration
    TheBestGuy_LogicGates = TheBestGuy.LogicGates
    ...
    outfile = open(sys_path + '/result/' + sys_output_name + '_CheckPoint.txt', 'a')
    outfile.write('SS_0')
    outfile.write('\n')
